 ...
 */
package project.beryl.com.newfirebaseapplication.notification;
...
public final class MockDatabase {
    ...
    public static MessagingStyleCommsAppData getMessagingStyleData(Context context, Map otherUserChatMap) {
        return MessagingStyleCommsAppData.getInstance(context,otherUserChatMap);
    }
    ...
    public static class MessagingStyleCommsAppData extends MockNotificationData {
        ...
        public static MessagingStyleCommsAppData getInstance(Context context, Map otherUserChatMap) {
            if (sInstance == null) {
                sInstance = getSync(context,otherUserChatMap);
            }
            return sInstance;
        }

        private static synchronized MessagingStyleCommsAppData getSync(Context context, Map otherUserChatMap) {
            if (sInstance == null) {
                sInstance = new MessagingStyleCommsAppData(context, otherUserChatMap);
            }

            return sInstance;
        }

        private MessagingStyleCommsAppData(Context context,Map otherUserChatMap) {
            // Standard notification values:
            ...
            mMe =
                    new Person.Builder()
                            .setName(otherUserChatMap.get("name").toString())
                            .setKey(FirebaseAuth.getInstance().getCurrentUser().getUid())
                            .setUri("tel:9685996627")
          ...
          //  mMessages.add( new NotificationCompat.MessagingStyle.Message(otherUserChatMap.get("last_message").toString(), 1528490641998l, participant1).setData("image/png", resourceToUri(context, R.drawable.earth)));



            mMessages.add(new NotificationCompat.MessagingStyle.Message(otherUserChatMap.get("last_message").toString(), 1528490643998l, mMe));

           // mMessages.add(new NotificationCompat.MessagingStyle.Message("HEY, I see my house!", 1528490645998l, participant2));
